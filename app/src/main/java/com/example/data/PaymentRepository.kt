...
class PaymentRepository(private val context: Context) {
    ...
    companion object {
        ...
        fun attachActivity(activity: Activity) { hostActivity = WeakReference(activity) }
        fun currentActivity(): Activity? = hostActivity?.get()
    }
    ...
    fun launchCheckout(paymentOrder: RazorpayOrder, customerName: String, email: String, phone: String): Result<Unit> = runCatching { launchCheckout(hostActivity?.get() ?: error("Payment screen is not available."), paymentOrder, customerName, email, phone) }
    fun launchCheckout(activity: Activity, paymentOrder: RazorpayOrder, customerName: String, email: String, phone: String) {
        val checkout = Checkout(); checkout.setKeyID(paymentOrder.keyId)
        val options = JSONObject().apply { put("key", paymentOrder.keyId); put("order_id", paymentOrder.razorpayOrderId); put("amount", paymentOrder.amountPaise); put("currency", paymentOrder.currency); put("name", "BarmerEats"); put("description", "BarmerEats food order"); put("prefill", JSONObject().apply { put("name", customerName); put("email", email); put("contact", phone) }); put("theme", JSONObject().apply { put("color", "#7A1F1F") }) }
    ...
    suspend fun verifyRazorpayPayment(orderId: String, razorpayOrderId: String, razorpayPaymentId: String, signature: String): Result<Boolean> = runCatching {
        ...
        val data = result.data as? Map<*, *> ?: error("Invalid verification response."); data["verified"] == true
    }
    suspend fun initiateRazorpayPayment(amountInRupees: Double, orderId: String, customerEmail: String, customerPhone: String): PaymentResult = if (amountInRupees <= 0.0 || orderId.isBlank()) PaymentResult.Error(400, "Invalid payment amount or order ID.") else PaymentResult.Error(400, "Use the trusted Razorpay checkout flow.")
