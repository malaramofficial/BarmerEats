...
export const createOrder = onCall(async (request) => {
  ...
  const data = request.data as {
    ...
    deliveryAddress?: {addressLine?: string; lat?: number; lng?: number};
  ...
  if (!restaurantId || items.length === 0 || !allowedPaymentMethods.has(paymentMethod ?? "")) {
    throw new HttpsError("invalid-argument", "Invalid restaurant, cart or payment method.");
  }
  ...
  if (restaurant.isOpen !== true || restaurant.approved !== true) {
    throw new HttpsError("failed-precondition", "Restaurant is not currently accepting orders.");
  }

  const uniqueIds = new Set<string>();
  for (const item of items) {
    if (!item.menuItemId || !Number.isInteger(item.quantity) || item.quantity! < 1 || item.quantity! > 20) {
      throw new HttpsError("invalid-argument", "Invalid cart item quantity.");
    }
  ...
  for (const item of items) {
    ...
    if (!menuItem || menuItem.isAvailable !== true) {
      throw new HttpsError("failed-precondition", "A selected item is unavailable.");
    }
  ...
  if (!address.addressLine || typeof address.lat !== "number" || typeof address.lng !== "number") {
...
export const createRazorpayOrder = onCall(
  ...
  async (request) => {
    ...
    const paymentOrder = await razorpay.orders.create({
      ...
      currency: "INR",
      receipt: orderId,
      notes: {barmerEatsOrderId: orderId, customerId: request.auth.uid},
    });
